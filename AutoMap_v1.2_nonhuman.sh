...
usage() { echo "## ERROR: Usage: $0 [--vcf <string>] [--out <string>] [--common] [--id <string>] [--panel <string>] [--panelname <string>] [--DP <0-99>] [--binomial <0-1.0>] [--percaltlow <0-1.0>] [--percalthigh <0-1.0>] [--window <3-999>] [--windowthres <1-999>] [--minsize <0-99>] [--minvar <1-999>] [--minperc <0-100>] [--maxgap <0-1000Mb>] [--chrX] [--extend <0-100Mb>]. Exit." 1>&2; exit 1; }
...
if [ "$multivcf" == "Yes" ]; then
    ...
    do
        ...
        if [[ "$chrx" == "No" ]] && [[ "$panelname" != "NA" ]]; then
            bash $here/AutoMap_v1.2.sh --id $pat --vcf $out/$pat/$pat.individual.vcf --out $out --panel $panel --panelname $panelname --DP $DP --binomial $binomial --percaltlow $percaltlow --percalthigh $percalthigh --window $window --windowthres $windowthres --minsize $minsize --minvar $minvar --minperc $minperc --maxgap $maxgap --extend $extend
        fi
        if [[ "$chrx" == "No" ]] && [[ "$panelname" == "NA" ]]; then
            bash $here/AutoMap_v1.2.sh --id $pat --vcf $out/$pat/$pat.individual.vcf --out $out --DP $DP --binomial $binomial --percaltlow $percaltlow --percalthigh $percalthigh --window $window --windowthres $windowthres --minsize $minsize --minvar $minvar --minperc $minperc --maxgap $maxgap --extend $extend
        fi
...
if [ "$multivcf" == "No" ]; then
    ...
    do
        ...
        numb="$(grep -v "#" $output.tsv | wc -l)"
        tot="$(grep -v "#" $output.tsv | cut -f4 | awk '{s+=$1} END {print s}')"
        if [ "$tot" == "" ]; then
            tot=0
        fi
        ...
        file=$output.tsv
        tot="$(grep -v "#" $file | cut -f4 | awk '{s+=$1} END {print s}')"
        if [ "$tot" == "" ]; then
            tot=0
        fi
        echo "## INFO: $tot Mb are in Homozygous Regions" >> $file;
